...
SELECT *, MAX(acceptedNo) OVER (PARTITION BY id) max_accepted
FROM
	...
	WHERE c.year = 2021 AND c.use_wca_registration = 1 AND c.competitor_limit_enabled = 1
        ...
        AND (r.deleted_at IS NULL OR r.deleted_at > N.Nth_accepted_at OR (N.Nth_accepted_at IS NULL AND r.deleted_at IS NULL))) a
WHERE acceptedNo <= competitor_limit
ORDER BY id, acceptedNo
)
;

# Join payment information to these registrations and calculate the longest registration time from each competition:
SELECT id competitionId, countryId, competitor_limit, entry_fee, COUNT(paid_at) paid_online, MAX(reg_time) fill_time 
FROM (SELECT r.*, p.created_at paid_at, 
	TIMESTAMPDIFF(SECOND, r.registration_open, IF(accepted_at < p.created_at, accepted_at, IF(p.created_at is NULL, r.created_at, p.created_at))) reg_time
...
WHERE refunded_registration_payment_id IS NULL
	AND max_accepted = competitor_limit) reg_info2
GROUP BY id
ORDER BY fill_time;
